...
for nrun = 1:Nruns
    ...
    real_sout_ratios(nrun,1) = mean(mean(vars_real.sout(:,1:size(vars_real.sout,2)/2)))/mean(mean(vars_real.sout(:,size(vars_real.sout,2)/2+1):size(vars_real.sout,2)/2));
    yoke_sout_ratios(nrun,1) = mean(mean(vars_yoke.sout(:,1:size(vars_yoke.sout,2)/2)))/mean(mean(vars_yoke.sout(:,size(vars_yoke.sout,2)/2+1):size(vars_yoke.sout,2)/2));
